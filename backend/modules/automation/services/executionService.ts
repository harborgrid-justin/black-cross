    ...
    try {
      ...
      for (const action of sortedActions) {
        ...
        execution.actions_executed.push({
          ...
          start_time: new Date(Date.now() - (actionResult.duration || 0)),
          end_time: new Date(),
          duration: actionResult.duration || 0,
          output: 'output' in actionResult ? actionResult.output : undefined,
        ...
        if (actionResult.success) {
          ...
          if ('output' in actionResult && actionResult.output) {
          ...
          execution.errors.push({
            action_id: action.id,
            error_message: 'error' in actionResult ? actionResult.error : 'Unknown error',
            timestamp: new Date(),
          });
