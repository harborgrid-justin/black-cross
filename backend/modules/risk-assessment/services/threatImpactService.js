...
const ThreatImpact = require('../models/ThreatImpact');
...
class ThreatImpactService {
   ...
   */
  async analyzeThreatImpact(threatId, impactData) {
    try {
   ...
   */
  calculateOverallImpactScore(dimensions) {
   ...
   * @returns {Promise<Object>} Threat impact analysis
   */
  async getThreatImpact(threatId) {
    try {
   ...
   */
  async getImpactAnalyses(filters = {}) {
    try {
      const query = {};
   ...
   */
  calculateFinancialImpact(impactData) {
    const factors = {
...
module.exports = new ThreatImpactService();
