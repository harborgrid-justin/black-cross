...
class TaxonomyService {
   ...
   */
  async createTaxonomy(taxonomyData: any) {
    try {
   ...
   */
  async updateTaxonomy(taxonomyId: any, updateData: any) {
    try {
   ...
   */
  async getTaxonomy(taxonomyId: any) {
    try {
      const taxonomy = await Taxonomy.findOne({ id: taxonomyId });
      if (!taxonomy) {
        throw new Error('Taxonomy not found');
   ...
   */
  async deleteTaxonomy(taxonomyId: any) {
    try {
   ...
   * @returns {Promise<Object>} Updated taxonomy
   */
  async addCategory(taxonomyId: any, categoryData: any) {
    try {
   ...
   * @returns {Promise<Object>} Updated taxonomy
   */
  async removeCategory(taxonomyId: any, categoryId: any) {
    try {
   ...
   * @returns {Promise<Object>} Updated taxonomy
   */
  async addFrameworkMapping(taxonomyId: any, mappingData: any) {
    try {
   ...
   */
  async exportTaxonomy(taxonomyId: any, format: any = 'json') {
    try {
      const taxonomy = await Taxonomy.findOne({ id: taxonomyId });
      if (!taxonomy) {
        throw new Error('Taxonomy not found');
   ...
   */
  async importTaxonomy(taxonomyData: any) {
    try {
