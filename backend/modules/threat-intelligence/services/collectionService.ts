...
import deduplicationUtils from '../utils/deduplication';

const { generateThreatHash, isDuplicate } = deduplicationUtils;
class CollectionService {
   ...
   */
  async collectThreat(threatData: any) {
    try {
   ...
   */
  async updateExistingThreat(existingThreat: any, newData: any) {
    try {
      // Merge indicators
   ...
   */
  async batchCollect(threatDataArray: any) {
    try {
   ...
   */
  async streamThreats(filters: Record<string, any> = {}) {
    try {
   ...
   */
  validateThreatData(threatData: any) {
