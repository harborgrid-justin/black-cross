...
import { METRICS, CONTENT_TYPE } from '../constants';
...
interface MetricsStore {
  ...
  http_request_duration_seconds: DurationRecord[];
  http_requests_in_progress: number;
 ...
 */
function incrementRequestCounter(method: string, path: string, status: number): void {
 ...
 */
function recordRequestDuration(method: string, path: string, duration: number): void {
  ...
  if (metrics.http_request_duration_seconds.length > METRICS.MAX_DURATION_RECORDS) {
...
function metricsMiddleware(req: Request, res: Response, next: NextFunction): void {
  ...
  res.send = function sendWrapper(data: any): Response {
 ...
 */
function getMetricsPrometheus(req: Request, res: Response): void {
  const lines: string[] = [];
  ...
  res.set('Content-Type', CONTENT_TYPE.PROMETHEUS);
  res.send(`${lines.join('\n')}\n`);
}
 ...
 */
function getMetricsJson(req: Request, res: Response): void {
  const memUsage = process.memoryUsage();
